		...
		public function index() {
			...
			if ( request()->ajax() ) {
				...
				$table->editColumn( 'clinic.nickname', function( $row ) {
					return $row->clinic ? ucwords($row->clinic->nickname) : '';
				} );
				...
				$table->editColumn( 'contact_person.first_name', function( $row ) {
					return $row->contact_person ? ucwords($row->contact_person->first_name) : '';
				} );
				...
				$table->editColumn( 'storefront', function( $row ) {
					if ( $row->storefront ) {
						return '<a href="' . asset( env( 'UPLOAD_PATH' ) . '/' . $row->storefront ) . '" target="_blank">
						<img src="' . asset( env( 'UPLOAD_PATH' ) . '/thumb/' . $row->storefront ) . '"/>';
					};
				} );
